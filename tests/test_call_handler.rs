...
#[test]
fn no_carrier_in_cooldown_does_not_allow_duplicate_notification() {
    // After commit_call the state is Cooldown. The modem sends NO CARRIER as a
    // result of our ATH (hang-up). That NO CARRIER must not reset state to Idle,
    // otherwise the very next RING fires a second notification.
    let (mut h, mut modem, mut messenger, mut sender) = make_handler();
    h.handle_urc("RING", &mut modem, &mut messenger, &mut sender);
    h.handle_urc(
        r#"+CLIP: "+1",129,"",,"",0"#,
    ...
    // State is now Cooldown; modem sends NO CARRIER after ATH
    h.handle_urc("NO CARRIER", &mut modem, &mut messenger, &mut sender);
    // A new RING within cooldown must still be suppressed
    h.handle_urc("RING", &mut modem, &mut messenger, &mut sender);
    h.handle_urc(
        r#"+CLIP: "+1",129,"",,"",0"#,
        &mut modem,
    ...
    assert_eq!(
        modem.hang_up_count, 1,
        "only the first call should trigger hang-up"
    );
    assert_eq!(
        messenger.sent_count(),
        1,
        "only one IM notification expected"
    );
}

#[test]
fn second_clip_while_ringing_uses_first() {
    // Some modems send two +CLIP lines per call. Only the first commit should fire;
    // the second arrives after state has moved to Cooldown and must be a no-op.
    let (mut h, mut modem, mut messenger, mut sender) = make_handler();
    h.handle_urc("RING", &mut modem, &mut messenger, &mut sender);
    h.handle_urc(
        r#"+CLIP: "+10001",129,"",,"",0"#,
    ...
    assert_eq!(modem.hang_up_count, 1, "ATH must fire exactly once");
    assert_eq!(messenger.sent_count(), 1, "only one IM notification");
    assert!(
        messenger.contains_sent("+10001") || messenger.contains_sent("10001"),
        "first CLIP number must win: {:?}",
        messenger.last_sent()
    );
}
