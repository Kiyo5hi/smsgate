...
fn encode_gsm7_body(body: &str) -> Option<Vec<u8>> {
    ...
    for ch in body.chars() {
        let count = encode_gsm7_char(ch as u32, &mut buf);
        if count == 0 {
            return None;
        }
...
fn encode_bcd_phone(phone: &str, out: &mut Vec<u8>) {
    let international = phone.bytes().any(|byte| byte == b'+');
    let digit_count = phone.bytes().filter(u8::is_ascii_digit).count();
    ...
    for byte in phone.bytes().filter(u8::is_ascii_digit) {
        ...
        if let Some(previous) = low.take() {
            out.push((digit << 4) | previous);
        } else {
            low = Some(digit);
        }
    }
    if let Some(previous) = low {
        out.push(0xF0 | previous);
    }
}

fn bcd_phone_field_len(phone: &str) -> usize {
    2 + phone.bytes().filter(u8::is_ascii_digit).count().div_ceil(2)
}
...
pub fn count_sms_parts(body: &str, max_parts: usize) -> usize {
    ...
    let parts = count_gsm7_parts(body).unwrap_or_else(|| count_ucs2_parts(body));
    if parts <= max_parts {
        parts
    } else {
        0
...
fn count_ucs2_parts(body: &str) -> usize {
    let total = body.chars().map(char::len_utf16).sum::<usize>();
    if total <= 70 {
        return 1;
    }
    let mut parts = 1;
