...
// RFC-0159: dumpBriefSince — entries with unixTimestamp >= sinceUnix, newest-first.
String SmsDebugLog::dumpBriefSince(uint32_t sinceUnix) const
{
    if (count_ == 0)
        return String("(no SMS logged yet)");
    ...
    for (size_t i = 0; i < count_; i++)
    {
        ...
        if (e.unixTimestamp == 0 || e.unixTimestamp < sinceUnix)
            continue;
    ...
    if (matched == 0)
        return String("(no entries since that time)");
    return out;
}
